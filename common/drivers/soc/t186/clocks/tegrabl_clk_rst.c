		...
		enum tegrabl_clk_src_id_t clk_src)
{
	...
	if (!TEGRABL_CLK_CHECK_SRC_ID(clk_src))
	...
	if (check_clk_src_enable(clk_src) == false)
		return TEGRABL_ERROR(TEGRABL_ERR_SOURCE_NOT_STARTED, 1);
		...
		uint32_t *rate_set_khz)
{
	...
	enum tegrabl_clk_src_id_t curr_clk_src;
	tegrabl_error_t err = TEGRABL_NO_ERROR;

	if (!module_support(module, instance) || !rate_set_khz)
		...
		uint32_t *rate_khz)
{
	...
	if (!module_support(module, instance) || !rate_khz)
