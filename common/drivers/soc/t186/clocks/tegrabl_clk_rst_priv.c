...
struct module_car_info g_module_carinfo[TEGRABL_MODULE_NUM] = {
	...
	[TEGRABL_MODULE_DVFS] = CLOCKINFO_DATA(dvfs),
...
uint32_t get_divider(uint32_t src_rate, uint32_t module_rate,
		enum tegrabl_clk_div_type divtype)
{
	int sr = src_rate;
	int mr = module_rate;
	...
	if (divtype == TEGRABL_CLK_DIV_TYPE_FRACTIONAL) {
		...
		int div1 = 2 * (sr/mr - 1);
		int div2 = div1;
		while (((sr << 1)/(div1 + 2)) > mr) {
			div2 = div1;
			div1++;
		}
		uint32_t diff1 = ABS((sr << 1)/(div1 + 2) - mr);
		uint32_t diff2 = ABS((sr << 1)/(div2 + 2) - mr);
...
uint32_t get_clk_rate_khz(
		...
		uint32_t div,
		enum tegrabl_clk_div_type div_type)
{
	if (div_type == TEGRABL_CLK_DIV_TYPE_FRACTIONAL)
		return (src_rate_khz << 1) / (div + 2);
	else
		return src_rate_khz / (div + 1);
}

int get_src_idx(
		enum tegrabl_clk_src_id_t *src_list,
		...
		uint32_t div, uint32_t src_idx)
{
	...
	if (module == TEGRABL_MODULE_SDMMC) {
		...
		if (0 == NV_DRF_VAL(CLK_RST_CONTROLLER, PLLC4_BASE,
	...
	tegrabl_module_t module, bool enable, void *priv_data)
{
	...
	case TEGRABL_MODULE_XUSB_HOST:
		NV_CLK_RST_UPDATE_OFFSET(CLK_RST_CONTROLLER_CLK_OUT_ENB_XUSB_0,
			(enable << 2));
		break;
		...
		uint8_t instance)
{
	...
	return ((reg & (0x1 << pcar_info->bit_offset)) != 0U);
}
		...
		void *priv_data)
{
	...
	if ((pclk_info->clk_enb_set_reg == 0) && (pclk_info->clk_src_reg == 0))
