...
#define NV_FUSE_WRITE(reg, val) NV_WRITE32((fuse_base_address + reg), val)
...
#define SBKKEY_SIZE_BYTES 16
#define KEKKEY_SIZE_BYTES 32
#define EKKEY_SIZE_BYTES 32
#define KGKKEY_SIZE_BYTES 16
#define KEK0KEY_SIZE_BYTES 16
#define ODMID_SIZE_BYTES 8
...
#define FUSE_RESERVED_SW_MASK 0xFF
#define FUSE_RESERVED_SW_SHIFT 4
#define FUSE_RESERVED_BOOT_DEVICE_MASK 0x7
#define FUSE_RESERVED_BOOT_DEVICE_SHIFT 0
#define FUSE_RESERVED_IGNORE_STRAP_MASK 0x1
#define FUSE_RESERVED_IGNORE_STRAP_SHIFT 3
#define FUSE_RESERVED_APB2JTAG_LOCK_MASK 0x8
#define FUSE_BOOT_SECURITY_INFO_SECURE_MASK 0x7
...
static bool tegrabl_fuse_apb2jtag_lock_status(void)
{
	...
	if ((val & FUSE_RESERVED_APB2JTAG_LOCK_MASK) != 0)
		return true;
	else
		return false;
}
...
uint32_t tegrabl_fuse_get_bootrom_patch_version(void)
{
	...
	original_visibility = tegrabl_set_fuse_reg_visibility(true);
	val =  NV_FUSE_READ(FUSE_BOOTROM_PATCH_VERSION_0) & 0xFF;
	tegrabl_set_fuse_reg_visibility(original_visibility);
...
tegrabl_error_t tegrabl_fuse_query_size(uint8_t type, uint32_t *size)
{
	...
	case FUSE_DENVER_NV_MTS_RATCHET:
	...
	const uint32_t nbytes)
{
	...
	if ((nbytes == 0) || (regaddress == 0) || (pbyte == NULL)) {
		err = TEGRABL_ERROR(TEGRABL_ERR_INVALID, 0);
		return err;
	}
	regdata = 0;
	...
	uint8_t type, uint32_t *buffer, uint32_t size)
{
	...
	tegrabl_error_t err = TEGRABL_NO_ERROR;
	uint32_t reg_data = 0;
	tegrabl_set_fuse_reg_visibility(true);
	if (buffer == NULL) {
	...
	case FUSE_SKU_INFO:
		...
		*buffer = reg_data & FUSE_SKU_INFO_0_READ_MASK;
		break;
	case FUSE_CPU_SPEEDO0:
	...
	case FUSE_CCPLEX_DFD_ACCESS_DISABLE:
		*buffer = NV_FUSE_READ(FUSE_DENVER_DFD_ACCESS_DISABLE_0);
		break;
	case FUSE_DENVER_NV_MTS_RATCHET:
		*buffer = NV_FUSE_READ(FUSE_DENVER_NV_MTS_RATCHET_0);
		break;
	default:
