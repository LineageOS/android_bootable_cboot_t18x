...
#define TEGRABL_MB1BCT_MB2_PARAM_VERSION	6
...
typedef enum tegrabl_platform_config_type {
...
} tegrabl_platform_config_type_t;
...
typedef enum tegrabl_mb1bct_controller_id {
	TEGRABL_MB1BCT_CONTROLLER_GENERIC,
	TEGRABL_MB1BCT_CONTROLLER_I2C,
} tegrabl_mb1bct_controller_id_t;
...
struct tegrabl_feature_fields {
	...
	union {
		uint64_t data2;
		struct {
			uint64_t abi_version:16; /* Bits 15-0 */
		};
	};
	union {
		uint64_t data3;
...
struct tegrabl_mb1_bct {
	...
	TEGRABL_DECLARE_ALIGNED(uint8_t rsvd_for_mb2_bct_params[1060], 8);
